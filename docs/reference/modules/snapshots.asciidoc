...
Assuming that the shared filesystem is mounted to `/mount/backups/my_backup`, the following setting could be added to
...
`https:`, and `ftp:` URLs have to be whitelisted by specifying allowed URLs in the `repositories.url.allowed_urls` setting.
...
It returns a list of nodes where the repository was successfully verified, or an error message if the verification process failed.
...
By setting `include_global_state` to false it's possible to prevent the cluster global state being stored as part of
the snapshot. By default, the entire snapshot will fail if one or more indices participating in the snapshot don't have
all their primary shards available. This behaviour can be changed by setting `partial` to `true`.
...
The snapshotting process is executed in non-blocking fashion. All indexing and searching operations can continue to be
...
will be present in the snapshot. The snapshot process starts immediately for the primary shards that have been started
...
Only one snapshot process can be executed in the cluster at any time. While a snapshot of a particular shard is being
...
When a repository is deleted, Elasticsearch removes only the reference to the location where the repository is storing
...
should be restored and/or prevent global cluster state from being restored by using `indices` and
...
The restore operation can be performed on a functioning cluster. However, an existing index can be restored only if its
...
snapshots of all their shards available. It can occur if some shards failed to snapshot, for example. It is still possible to
...
Please note, that some settings such as `index.number_of_shards` cannot be changed during restore operations.
...
the new cluster has enough capacity to store all indices in the snapshot. It's possible to change indices settings
during restore to reduce the number of replicas, which can help with restoring snapshots into a smaller cluster. It's also
possible to select only a subset of the indices using the `indices` parameter.  Prior to version 1.5.0 elasticsearch
...
if the new cluster doesn't contain nodes with attributes appropriate for allocation of the index being restored, such an
...
operations support the `wait_for_completion` parameter that would block clients until the operation is completed. This is
...
Please note the snapshot info operation uses the same resources and thread pool as the snapshot operation. So,
...
While the snapshot info method returns only basic information about the snapshot in progress, the snapshot status returns a
...
monitoring services can be used to monitor the state of a restore. When a restore operation is executed the cluster
...
state. Once recovery of primary shards is completed, Elasticsearch is switches to the standard replication process that
creates the required number of replicas; at this moment, the cluster switches to the `yellow` state. Once all required replicas
